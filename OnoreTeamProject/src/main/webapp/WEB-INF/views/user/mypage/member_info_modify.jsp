...
function check_phone() {
    ...
    var regPhone = /^01([0]?)-?([0-9]{4})-?([0-9]{4})$/; // - 생략하고 01?(3글자) 방식으로 나머지 적용
...
$(document).ready(function() {
	  $("#info_modify_form").submit(function(event) {
	    event.preventDefault();
	    if (check_email() && check_phone()) {
	      $.ajax({
	        type: "POST",
	        url: "memberInfoModify",
	        data: $("#info_modify_form").serialize(),
		...
		<div class="info_modify_div">
			<div class="info_modify_title_div">
				...
				<div class="info_modify_birth_date_div">
					...
					<div class="info_modify_input_div">
						<input id="mem_birth_date" type="text" name="mem_birth_date" value="${member.mem_birth_date }" readonly>
					</div>
				</div>

				<div class="info_modify_gender_div">
					<div class="info_modify_a_div">
						<a>성별</a>
					</div>
					<div class="info_modify_input_radio_div">
						<input id="mem_gender" type="radio" name="mem_gender" value="${member.mem_gender eq 'M'}" checked disabled><a>남성</a> 
						<input id="mem_gender" type="radio" name="mem_gender" value="${member.mem_gender eq 'F'}" checked disabled><a>여성</a>
					</div>
				</div>
				...
				<div class="info_modify_check_sms_div">
					<div class="info_modify_input_check_div">
						<c:choose>
						<c:when test="${member.mem_sms_check eq 'Y'}">
							<input class="law_check_box" id="mem_sms_check" type="checkbox" name="mem_sms_check" checked value="Y">
						</c:when>
						<c:otherwise>
				...
				<div class="info_modify_check_email_div">
					<div class="info_modify_input_check_div">
						<c:choose>
						<c:when test="${member.mem_email_check eq 'Y'}">
							<input class="law_check_box" id="mem_email_check" type="checkbox" name="mem_email_check" checked value="Y">
						</c:when>
						<c:otherwise>
