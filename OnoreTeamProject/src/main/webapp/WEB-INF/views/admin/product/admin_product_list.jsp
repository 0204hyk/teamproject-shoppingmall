...
<body>
	...
	<div class="notice-content shadow">
		<div class="notice-title"><h1>상품 목록</h1></div>
				...
				<table class="notice-board table table-hover">
					...
					<tbody>
						<c:forEach items="${productList}" var="product">
							<tr>
								<td style="width: 50px;"><input type="checkbox" class="row-check" name="row_check" value="${product.product_num}"/></td>
								<td style="width: 80px;">${product.product_num}</td>
								<td><a href="javascript:movePageByGet('<%=request.getContextPath()%>/admin/notice/modify?notice_num=${notice.notice_num}')">${product.product_name}</a></td>
								<td style="width: 200px;">${product.creationDateTime}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
